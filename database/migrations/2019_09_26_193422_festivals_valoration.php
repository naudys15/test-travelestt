...
class FestivalsValoration extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('festivalvaloration', function (Blueprint $table) {
            $table->increments('id')->comment('id de la valoracion del festival');
            $table->integer('id')->unsigned()->comment('id del festival');
            $table->foreign('id')->references('id')->on('festival');
    ...
    {
        Schema::dropIfExists('festivalvaloration');
    }
}
