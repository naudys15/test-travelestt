...
class RoutesValoration extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('routevaloration', function (Blueprint $table) {
            $table->increments('id')->comment('id de la valoracion de la ruta');
            $table->integer('id')->unsigned()->comment('id de la ruta');
            $table->foreign('id')->references('id')->on('route');
    ...
    {
        Schema::dropIfExists('routevaloration');
    }
}
