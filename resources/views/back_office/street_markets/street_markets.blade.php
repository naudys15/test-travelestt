    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 21);
                $edit = permissionValid($permissions, 22);
                $delete = permissionValid($permissions, 24);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage($('html').attr('lang'))">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="erasePlace('street_markets')" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                @else
