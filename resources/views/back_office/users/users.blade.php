    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 29);
                $edit = permissionValid($permissions, 30);
                $delete = permissionValid($permissions, 32);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage($('html').attr('lang'))">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="eraseUser()" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                @else
