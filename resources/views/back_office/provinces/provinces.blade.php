    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 65);
                $edit = permissionValid($permissions, 66);
                $delete = permissionValid($permissions, 68);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage()">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="erasePlace('provinces')" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                 @else
                ...
                <table id="table_content" @if (app()->getLocale() == 'es') data-locale="es-ES" @elseif (app()->getLocale() == 'en') data-locale="en-EN" @elseif (app()->getLocale() == 'fr') data-locale="fr-FR"@endif>
                    <thead>
                        <tr>
                        <th width="5%" data-field="check" data-checkbox="true">Id</th>
                        </tr>
                    </thead>
                </table>
            </div>
