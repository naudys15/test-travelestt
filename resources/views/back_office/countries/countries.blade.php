    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 61);
                $edit = permissionValid($permissions, 62);
                $delete = permissionValid($permissions, 64);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage($('html').attr('lang'))">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="erasePlace('countries')" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                @else
