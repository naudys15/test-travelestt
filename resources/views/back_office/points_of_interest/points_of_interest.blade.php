    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 17);
                $edit = permissionValid($permissions, 18);
                $delete = permissionValid($permissions, 20);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage($('html').attr('lang'))">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="erasePlace('points_of_interest')" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                @else
