    ...
    <div class="panel">
        <div class="panel-body">
            @php
                ...
                $add = permissionValid($permissions, 25);
                $edit = permissionValid($permissions, 26);
                $delete = permissionValid($permissions, 28);
            @endphp
            <div class="col-xs-12" id="setLanguage" onclick="setLanguage($('html').attr('lang'))">
                ...
                @endif
                @if($delete)
                    <button id="btn_delete_rows" onclick="erasePlace('routes')" class="btn btn-default" disabled><i class="ti-trash"></i> {{ __('language.delete') }}</button>
                @else
