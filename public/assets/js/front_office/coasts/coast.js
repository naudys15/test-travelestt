...
$.ajax({
    async: false,
    url: location.origin+"/api/coasts/slug/"+location.href.split('/').pop(),
    type: 'GET',
    dataType: 'json',
    statusCode: {
        200: function(data) {
            ...
            if ($('#comment_form').length > 0) {
                $('#comment_form').attr('href', location.origin+'/api/coasts/'+resource.id+'/comment')
                $('#comment_form').attr('action', 'POST')
            }
            for (i = 0; i < resource.media.length; i++) {
                let imagen = location.origin+"/assets/images/coasts/"+resource.media[i];
                if (i == 0) {
            ...
            let type_sand = resource.characteristics.type_sand[0]
            if (resource.translations[language] != undefined ) {
                $('html>head>title').html(translations['coasts']+' | '+resource.translations[language].name)
                ...
                if (type_sand == 'white_sand') {
                    $('#place_sand').html(translations['white_sand'])
                }
                if (type_sand == 'yellow_sand') {
                    $('#place_sand').html(translations['white_sand'])
                }
                if (type_sand == 'green_sand') {
                    $('#place_sand').html(translations['green_sand'])
                }
                if (type_sand == 'black_sand') {
                    $('#place_sand').html(translations['black_sand'])
                }
                if (type_sand == 'fine_stone') {
                    $('#place_sand').html(translations['fine_sand'])
                }
                if (type_sand == 'thick_stone') {
                    $('#place_sand').html(translations['thick_stone'])
                }
                if (resource.characteristics.extras) {
                    var extras = ''
                    for(i = 0; i < resource.characteristics.extras.length; i++) {
                        if (data.message[0].characteristics.extras[i] == 'feet_lavatory') {
                            extras += '<div class="col-12 mb-3"><div class="d-flex"><i class="fas fa-shoe-prints fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['feet_lavatory']+'</p></div></div>'
                        }
                        if  (data.message[0].characteristics.extras[i] == 'handicapped_access') {
                            extras += '<div class="col-12 mb-3"><div class="d-flex"><i class="fab fa-accessible-icon fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['handicapped_access']+'</p></div></div>'
                        }
                        if (data.message[0].characteristics.extras[i] == 'showers') {
                            extras+='<div class="col-12 mb-3"><div class="d-flex"><i class="fas fa-shower fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['showers']+'</p></div></div>'
                        }
                        if (data.message[0].characteristics.extras[i] == 'lifeguards') {
                            extras += '<div class="col-12 mb-3"><div class="d-flex"><i class="fas fa-life-ring fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['lifeguards']+'</p></div></div>'
                        }
                        if (data.message[0].characteristics.extras[i] == 'watersports') {
                            extras += '<div class="col-12 mb-3"><div class="d-flex"><i class="fas fa-swimmer fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['watersports']+'</p></div></div>'
                        }
                        if (data.message[0].characteristics.extras[i] == 'chiringuito') {
                            extras+='<div class="col-12 mb-3"><div class="d-flex"><i class="fas fa-store fa-2x text-aqua mr-2"></i><p id="place_extra" class="text-secondary mb-0 align-self-center">'+translations['chiringuito']+'</p></div></div>'
                        }
                    }
                    $('#place_extras_content').html(extras)
                }
            } else {
                window.history.back()
            }
            ...
            $.ajax({
                async: false,
                url: location.origin+"/api/coasts/"+resource.id+"/comments",
                type: 'GET',
                dataType: 'json',
                statusCode: {
                    200: function(data) {
