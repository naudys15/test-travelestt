...
$.ajax({
    async: false,
    url: location.origin+"/api/experiences/slug/"+location.href.split('/').pop(),
    type: 'GET',
    dataType: 'json',
    ...
    statusCode: {
        200: function(data) {
            ...
            if ($('#comment_form').length > 0) {
                $('#comment_form').attr('href', location.origin+'/api/experiences/'+resource.id+'/comment')
                $('#comment_form').attr('action', 'POST')
            }
            for (i = 0; i < resource.media.length; i++) {
                var imagen = location.origin+"/assets/images/experiences/"+resource.media[i];
                if (i == 0) {
            ...
            if (resource.translations[language] != undefined ) {
                $('html>head>title').html(translations['experiences']+' | '+resource.translations[language].name)
            ...
            $.ajax({
                async: false,
                url: location.origin+"/api/experiences/"+resource.id+"/comments",
                type: 'GET',
                dataType: 'json',
                statusCode: {
                    200: function(data) {
