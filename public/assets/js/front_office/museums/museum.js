...
$.ajax({
    async: false,
    url: location.origin+"/api/museums/slug/"+location.href.split('/').pop(),
    type: 'GET',
    dataType: 'json',
    ...
    statusCode: {
        200: function(data) {
            ...
            if ($('#comment_form').length > 0) {
                $('#comment_form').attr('href', location.origin+'/api/museums/'+resource.id+'/comment')
                $('#comment_form').attr('action', 'POST')
            }
            for (i = 0; i < resource.media.length; i++) {
                var imagen = location.origin+"/assets/images/museums/"+resource.media[i]
                if (i == 0) {
            ...
            let place_type = resource.characteristics[0]
            if (resource.translations[language] != undefined ) {
                $('html>head>title').html(translations['museums']+' | '+resource.translations[language].name)
                ...
                if (place_type == 'decorative_arts') {
                    $('#place_type').html(translations['decorative_arts'])
                }
                if (place_type == 'classic') {
                    $('#place_type').html(translations['classic'])
                }
                if (place_type == 'ethnographic') {
                    $('#place_type').html(translations['ethnographic'])
                }
                if (place_type == 'musical') {
                    $('#place_type').html(translations['musical'])
                }
                if (place_type == 'modernist') {
                    $('#place_type').html(translations['modernist'])
                }
                if (place_type == 'fine_arts') {
                    $('#place_type').html(translations['fine_arts'])
                }
                if (place_type == 'historical') {
                    $('#place_type').html(translations['historical'])
                }
                if (place_type == 'contemporary') {
                    $('#place_type').html(translations['contemporary'])
                }
                if (place_type == 'natural_sciences') {
                    $('#place_type').html(translations['natural_sciences'])
                }
                if (place_type == 'maritime_and_naval') {
                    $('#place_type').html(translations['maritime_and_naval'])
                }
                if (place_type == 'anthropological') {
                    $('#place_type').html(translations['anthropological'])
                }
                if (place_type == 'technological_scientist') {
                    $('#place_type').html(translations['technological_scientist'])
                }
                if (place_type == 'military') {
                    $('#place_type').html(translations['military'])
                }
            } else {
                window.history.back()
            }
            ...
            $.ajax({
                async: false,
                url: location.origin+"/api/museums/"+resource.id+"/comments",
                type: 'GET',
                dataType: 'json',
                statusCode: {
                    200: function(data) {
