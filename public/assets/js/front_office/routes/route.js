setLanguage() // $(document).ready(function() {
$.ajax({
    async: false,
    url: location.origin+"/api/routes/slug/"+location.href.split('/').pop(),
    type: 'GET',
    dataType: 'json',
    headers: {
    ...
    statusCode: {
        200: function (data) {
            ...
            if ($('#comment_form').length > 0) {
                $('#comment_form').attr('href', location.origin+'/api/routes/'+resource.id+'/comment')
                $('#comment_form').attr('action', 'POST')
            }
            for (i = 0; i < resource.media.length; i++) {
                var imagen=location.origin+"/assets/images/routes/"+resource.media[i];
                if (i == 0) {
            ...
            if (resource.translations[language] != undefined) {
                $('html>head>title').html(translations['routes']+' | '+resource.translations[language].name)
            ...
            var mymap
            var marker
            var lat = resource.latitude_start
            var lng = resource.longitude_start
            mymap = L.map('map_destino').setView([lat, lng], 13)
            ...
            // agrego el marcador del inicio de la ruta
            marker = new L.marker([resource.latitude_start, resource.longitude_start], {
                draggable: false,
                autoPan: true,
                zoomControl: false,
                icon: myStartIcon,
            }).bindPopup("Coordenadas: <br> " + data.message[0].latitude_start + "," + data.message[0].longitude_start);
            mymap.addLayer(marker);
            var inicio_ruta = '<li class="timeline-item rounded ml-3 p-3"> <div class = "timeline-arrow" > </div> <h2 class="h5 mb-0">' + data.message[0].location.city.name + '</h2><span class="small text-gray"><i class="fa fa-clock-o mr-1"></i >5 km </span></li>';
            $('.timeline').append(inicio_ruta);

            // agrego los marcadores de las estaciones
            $.each(data.message[0].stations, function (index, value) {
                marker = new L.marker([value.latitude, value.longitude], {
                    draggable: false,
                    autoPan: true,
                    zoomControl: false,
                    icon: myIcon,
                }).bindPopup("Coordenadas: <br> " + value.latitude + "," + value.longitude);
                mymap.addLayer(marker);
                if (value[language] != undefined) {
                    var station_title = value[language];
                    // $('#place_name').html(resource.translations[language].name)
                    // $('#place_address').html(resource.translations[language].address)
                    // $('#place_short_description').html(resource.translations[language].short_description)
                } else {
                ...
                let timeline = '<li class="timeline-item rounded ml-3 p-3"> <div class = "timeline-arrow" > </div> <h2 class="h5 mb-0"> ' + station_title + ' </h2><span class="small text-gray"><i class="fa fa-clock-o mr-1"></i >5 km para próxima estación</span></li>'
                $('.timeline').append(timeline);
            })

            // agrego el marcador del final de la ruta
            marker = new L.marker([resource.latitude_end, resource.longitude_end], {
                draggable: false,
                autoPan: true,
                zoomControl: false,
                icon: myEndIcon,
            }).bindPopup("Coordenadas: <br> " + resource.latitude_end + "," + resource.longitude_end);
            mymap.addLayer(marker);
            var final_ruta = '<li class="timeline-item rounded ml-3 p-3"> <div class = "timeline-arrow" > </div> <h2 class="h5 mb-0">Final de la ruta</h2><span class="small text-gray"><i class="fa fa-clock-o mr-1"></i >5 km </span></li>';
            $('.timeline').append(final_ruta);
            var user = (!localStorage.getItem('travelesttUser')) ? false : JSON.parse(descrypt(localStorage.getItem('travelesttUser')))
            $.ajax({
                async: false,
                url: location.origin+"/api/routes/"+resource.id+"/comments",
                type: 'GET',
                dataType: 'json',
                statusCode: {
                    200: function(data) {
