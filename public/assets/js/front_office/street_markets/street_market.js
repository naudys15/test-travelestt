...
$.ajax({
    async: false,
    url: location.origin+"/api/street_markets/slug/"+location.href.split('/').pop(),
    type: 'GET',
    dataType: 'json',
    ...
    statusCode: {
        200: function(data) {
            ...
            if ($('#comment_form').length > 0) {
                $('#comment_form').attr('href', location.origin+'/api/street_markets/'+resource.id+'/comment')
                $('#comment_form').attr('action', 'POST')
            }
            for (i = 0; i < resource.media.length; i++) {
                var imagen = location.origin+"/assets/images/street_markets/"+resource.media[i];
                if (i == 0) {
            ...
            if (resource.translations[language] != undefined ) {
                $('html>head>title').html(translations['street_markets']+' | '+resource.translations[language].name)
                ...
                if (place_type == 'gastronomic') {
                    $('#place_type').html(translations['gastronomic'])
                }
                if (place_type == 'traditional') {
                    $('#place_type').html(translations['traditional'])
                }
                if (place_type == 'rastro') {
                    $('#place_type').html(translations['rastro'])
                }
                if (place_type == 'street_food_market') {
                    $('#place_type').html(translations['street_food_market'])
                }
                if (place_type == 'antiques') {
                    $('#place_type').html(translations['antiques'])
                }
                if (place_type == 'of_supplies') {
                    $('#place_type').html(translations['of_supplies'])
                }
            } else {
                window.history.back()
            }
            ...
            $.ajax({
                async: false,
                url: location.origin+"/api/street_markets/"+resource.id+"/comments",
                type: 'GET',
                dataType: 'json',
                statusCode: {
                    200: function(data) {
