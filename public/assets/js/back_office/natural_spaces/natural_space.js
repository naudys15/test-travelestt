...
loadCategoriesAndCharacteristics('natural_space')
...
if (Number.isInteger(id)) {
    $.ajax({
        url: location.origin+"/api/natural_spaces/"+id,
        type: 'GET',
        dataType: 'json',
        ...
        statusCode: {
            200: function(data) {
                ...
                if (setTitleToCity) {
                    $('html>head>title').html('Travelestt | '+resource.location.city.name+' | '+translations['edit_natural_space'])
                    $('#page-title h1').html(resource.location.city.name+' | '+translations['edit_natural_space'])
                } else {
                    $('html>head>title').html('Travelestt | '+translations['edit_natural_space'])
                    $('#page-title h1').html(translations['edit_natural_space'])
                }
                if (resource.media) {
                    for (i = 0; i < resource.media.length; i++) {
                        let place_img_url = location.origin+"/assets/images/natural_spaces/"+resource.media[i]
                        let mockFile = {
                ...
                console.log(resource.characteristics[0])
                $('#types_'+resource.characteristics[0]).prop('checked', true)
                var url = location.origin+'/'+language+'/'+resource.location.city.slug+'/'+translations['url_natural_spaces']+'/'+resource.slug
                $('#url').html(url)
                $('#url').attr('href', url)
    ...
    $(document).ready(function(){
        if (setTitleToCity) {
            $('html>head>title').html('Travelestt | '+$("#city option:selected").html()+' | '+translations['new_natural_space'])
            $('#page-title h1').html($("#city option:selected").html()+' | '+translations['new_natural_space'])
        } else {
            $('html>head>title').html('Travelestt | '+translations['new_natural_space'])
            $('#page-title h1').html(translations['new_natural_space'])
        }
...
$("#form_site").on('submit',function (e) {
    ...
    // if (url_place.indexOf('natural_space') > 0 || url_place.indexOf('night') > 0 || url_place.indexOf('museum') > 0 || url_place.indexOf('market') > 0) {
    //     validation_schedule_to_entities_with_schedule = 1;
    // }
    fields_in_form = ($('#slug').val() != "" && name != "" && shortDescription != "" && longDescription != "" && address != "" && metaTitle != "" && metaDescription != "" && $('#city option:selected').val() != "" && $('input:radio[name=types]:checked').val() != undefined)
    ...
    if (marker != undefined && files_in_dropzone >= 2 && files_in_dropzone <= 5 && fields_in_form == true) {
        ...
        message = translations['message_natural_space'];
