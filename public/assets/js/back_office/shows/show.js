...
if (Number.isInteger(id)) {
    $.ajax({
        url: location.origin+"/api/shows/"+id,
        type: 'GET',
        dataType: 'json',
        ...
        statusCode: {
            200: function(data) {
                ...
                if (setTitleToCity) {
                    $('html>head>title').html('Travelestt | '+resource.location.city.name+' | '+translations['edit_show'])
                    $('#page-title h1').html(resource.location.city.name+' | '+translations['edit_show'])
                } else {
                    $('html>head>title').html('Travelestt | '+translations['edit_show'])
                    $('#page-title h1').html(translations['edit_show'])
                }
                if (resource.media) {
                    for (i = 0; i < resource.media.length; i++) {
                        let place_img_url = location.origin+"/assets/images/shows/"+resource.media[i]
                        let mockFile = {
                ...
                var url = location.origin+'/'+language+'/'+resource.location.city.slug+'/'+translations['url_shows']+'/'+resource.slug
                $('#url').html(url)
                $('#url').attr('href', url)
    ...
    $(document).ready(function(){
        if (setTitleToCity) {
            $('html>head>title').html('Travelestt | '+$("#city option:selected").html()+' | '+translations['new_show'])
            $('#page-title h1').html($("#city option:selected").html()+' | '+translations['new_show'])
        } else {
            $('html>head>title').html('Travelestt | '+translations['new_show'])
            $('#page-title h1').html(translations['new_show'])
        }
...
$("#form_site").on('submit',function (e) {
    ...
    // if (url_place.indexOf('show') > 0 || url_place.indexOf('night') > 0 || url_place.indexOf('museum') > 0 || url_place.indexOf('market') > 0) {
    ...
    if (marker != undefined && files_in_dropzone >= 2 && files_in_dropzone <= 5 && fields_in_form == true) {
        ...
        message = translations['message_show'];
