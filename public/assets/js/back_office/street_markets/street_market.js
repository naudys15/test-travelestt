...
loadCategoriesAndCharacteristics('street_market')
...
if (Number.isInteger(id)) {
    $.ajax({
        url: location.origin+"/api/street_markets/"+id,
        type: 'GET',
        dataType: 'json',
        ...
        statusCode: {
            200: function(data) {
                ...
                if (setTitleToCity) {
                    $('html>head>title').html('Travelestt | '+resource.location.city.name+' | '+translations['edit_street_market'])
                    $('#page-title h1').html(resource.location.city.name+' | '+translations['edit_street_market'])
                } else {
                    $('html>head>title').html('Travelestt | '+translations['edit_street_market'])
                    $('#page-title h1').html(translations['edit_street_market'])
                }
                if (resource.media) {
                    for (i = 0; i < resource.media.length; i++) {
                        let place_img_url = location.origin+"/assets/images/street_markets/"+resource.media[i]
                        let mockFile = {
                ...
                var url = location.origin+'/'+language+'/'+resource.location.city.slug+'/'+translations['url_street_markets']+'/'+resource.slug
                $('#url').html(url)
                $('#url').attr('href', url)
    ...
    $(document).ready(function(){
        if (setTitleToCity) {
            $('html>head>title').html('Travelestt | '+$("#city option:selected").html()+' | '+translations['new_street_market'])
            $('#page-title h1').html($("#city option:selected").html()+' | '+translations['new_street_market'])
        } else {
            $('html>head>title').html('Travelestt | '+translations['new_street_market'])
            $('#page-title h1').html(translations['new_street_market'])
        }
...
$("#form_site").on('submit',function (e) {
    ...
    if (marker != undefined && files_in_dropzone >= 2 && files_in_dropzone <= 5 && fields_in_form == true) {
        ...
        message = translations['message_street_market'];
