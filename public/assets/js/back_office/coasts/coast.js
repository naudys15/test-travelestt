...
if (Number.isInteger(id)) {
    $.ajax({
        async: false,
        url: location.origin+"/api/coasts/"+id,
        type: 'GET',
        dataType: 'json',
        ...
        statusCode: {
            200: function(data) {
                ...
                if (setTitleToCity) {
                    $('html>head>title').html('Travelestt | '+resource.location.city.name+' | '+translations['edit_coast'])
                    $('#page-title h1').html(resource.location.city.name+' | '+translations['edit_coast'])
                } else {
                    $('html>head>title').html('Travelestt | '+translations['edit_coast'])
                    $('#page-title h1').html(translations['edit_coast'])
                }
                if (resource.media) {
                    for (i = 0; i < resource.media.length; i++) {
                        let place_img_url = location.origin+"/assets/images/coasts/"+resource.media[i]
                        let mockFile = {
                ...
                // $('#type_sand_'+resource.type_sand).prop('checked',true)
                if (resource.characteristics.type_sand) {
                    $('input#type_sand_'+resource.characteristics.type_sand).prop('checked',true)
                }
                if (resource.characteristics.extras) {
                    for (let i=0; i <= resource.characteristics.extras.length; i++) {
                        $('input#extras_'+resource.characteristics.extras[i]).prop('checked',true)
                    }
                }
                if (resource.characteristics.stamps) {
                    for(let i = 0; i <= resource.characteristics.stamps.length; i++){
                        $('input#stamps_'+resource.characteristics.stamps[i]).prop('checked',true)
                    }
                }
                var url = location.origin+'/'+language+'/'+resource.location.city.slug+'/'+translations['url_coasts']+'/'+resource.slug
                $('#url').html(url)
                $('#url').attr('href', url)
    ...
    $(document).ready(function(){
        if (setTitleToCity) {
            $('html>head>title').html('Travelestt | '+$("#city option:selected").html()+' | '+translations['new_coast'])
            $('#page-title h1').html($("#city option:selected").html()+' | '+translations['new_coast'])
        } else {
            $('html>head>title').html('Travelestt | '+translations['new_coast'])
            $('#page-title h1').html(translations['new_coast'])
        }
...
$("#form_site").on('submit',function (e) {
    ...
    // }
    fields_in_form = ($('#slug').val() != "" && name != "" && shortDescription != "" && longDescription != "" && address != "" && metaTitle != "" && metaDescription != "" && $('input:radio[name=type_sand]:checked').val() != undefined && $('#city option:selected').val() != "")
    var extras = []
    if ($('input.extras:checkbox:checked').length > 0) {
        $('input.extras:checkbox:checked').map(function(){
            extras.push(this.value)
        })
    }
    var stamps = []
    if ($('input.stamps:checkbox:checked').length > 0) {
        $('input.stamps:checkbox:checked').map(function(){
            stamps.push(this.value)
        })
    }
    ...
    if (marker != undefined && files_in_dropzone >= 2 && files_in_dropzone <= 5 && fields_in_form == true) {
        ...
        message = translations['message_coast']
        formData.append('characteristics[]', $('input:radio[name=type_sand]:checked').val())
        for (var i = 0; i < extras.length; i++) {
            formData.append('characteristics[]', extras[i])
        }
        for (var i = 0; i < stamps.length; i++) {
            formData.append('characteristics[]', stamps[i])
        }
