...
if (Number.isInteger(id)) {
    $.ajax({
        url: location.origin+"/api/experiences/"+id,
        type: 'GET',
        dataType: 'json',
        ...
        statusCode: {
            200: function(data) {
                ...
                if (setTitleToCity) {
                    $('html>head>title').html('Travelestt | '+resource.location.city.name+' | '+translations['edit_experience'])
                    $('#page-title h1').html(resource.location.city.name+' | '+translations['edit_experience'])
                } else {
                    $('html>head>title').html('Travelestt | '+translations['edit_experience'])
                    $('#page-title h1').html(translations['edit_experience'])
                }
                if (resource.media) {
                    for (i = 0; i < resource.media.length; i++) {
                        let place_img_url = location.origin+"/assets/images/experiences/"+resource.media[i]
                        let mockFile = {
                ...
                var url = location.origin+'/'+language+'/'+resource.location.city.slug+'/'+translations['url_experiences']+'/'+resource.slug
                $('#url').html(url)
                $('#url').attr('href', url)
    ...
    $(document).ready(function(){
        if (setTitleToCity) {
            $('html>head>title').html('Travelestt | '+$("#city option:selected").html()+' | '+translations['new_experience'])
            $('#page-title h1').html($("#city option:selected").html()+' | '+translations['new_experience'])
        } else {
            $('html>head>title').html('Travelestt | '+translations['new_experience'])
            $('#page-title h1').html(translations['new_experience'])
        }
...
$("#form_site").on('submit',function (e) {
    ...
    if (marker != undefined && files_in_dropzone >= 2 && files_in_dropzone <= 5 && fields_in_form == true) {
        ...
        message = translations['message_experience'];
