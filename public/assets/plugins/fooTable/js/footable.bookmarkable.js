(function ($, w, undefined) {
    ...
    function addFootableStatusData(ft, event) {
        var total = $(ft.table).find("tbody").find("tr:not(.footable-row-detail, .footable-filtered)").length;
        $(ft.table).data("status_num_total", total);

        var num = $(ft.table).find("tbody").find("tr:not(.footable-row-detail)").filter(":visible").length;
        $(ft.table).data("status_num_shown", num);
         ...
         if (ft.options.bookmarkable.enabled) {
             ...
             $(ft.table).bind({
                 'footable_initialized': function(){
                     var id     = ft.table.id;
                     var q_filter   = $_HASH(id + '_f');
                     var q_page_num = $_HASH(id + '_p');
                     var q_sorted   = $_HASH(id + '_s');
                     var q_desc     = $_HASH(id + '_d');
                     var q_expanded = $_HASH(id + '_e');
