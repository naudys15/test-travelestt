...
MANIPATH = os.path.join(SCRIPT_PATH, '..', 'manifest.json')
...
manifile = open(MANIPATH, 'r')
manidata = json.loads(manifile.read())
manifile.close()
print "Load Manifest, Icons: %s" % ( len(manidata['icons']) )

build_data = copy.deepcopy(manidata)
build_data['icons'] = []

font_name = manidata['name']
m.update(font_name + ';')
m.update(manidata['prefix'] + ';')
...
if build_hash == manidata.get('build_hash'):
  print "Source files unchanged, did not rebuild fonts"

else:
  manidata['build_hash'] = build_hash
  ...
  manidata['icons'] = sorted(manidata['icons'], key=lambda k: k['name'])
  build_data['icons'] = sorted(build_data['icons'], key=lambda k: k['name'])

  print "Save Manifest, Icons: %s" % ( len(manidata['icons']) )
  f = open(MANIPATH, 'w')
  f.write( json.dumps(manidata, indent=2, separators=(',', ': ')) )
  f.close()
